...
var generateOrgUnit = function generateOrgUnit(orgUnitId){
    ...
    for(var i =0; i<consts.ROLE_IDS.length; i++){
        ...
        for (var j = 0; j<role.tools.length; j++){
            var tool = role.tools[j];
            allData[orgUnitId][role.id][tool.id] = {};
            var date = Date.now();
            for(var k = date - consts.MS_PER_YEAR; k <= date; k += consts.MS_PER_DAY){
                allData[orgUnitId][role.id][tool.id][k] = Math.floor((Math.random() * 100 * tool.useLevel * role.useLevel) + 1).toString();
            }
        }
    }
...
module.exports = {
    'generate': generate,
    'generateOrgUnit': generateOrgUnit
};